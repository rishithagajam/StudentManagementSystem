...
public class AuthoritiesQueryConstants {
   ...
   */
  public static final String INSERT_AUTHORITY = "INSERT INTO authority (mail, name, role,contact) VALUES (?, ?, ?, ?)";

  /**
   * SQL query to update an existing authority record in the 'authority' table.
   * Parameters: A_name, A_role, A_contact, A_mail
   */
  public static final String UPDATE_AUTHORITY = "UPDATE authority SET name=?,role=?, contact=? WHERE mail=?";

  /**
   * SQL query to delete an authority record from the 'authority' table by email.
   * Parameters: A_mail
   */
  public static final String DELETE_AUTHORITY = "DELETE FROM authority WHERE mail=?";

  /**
   * SQL query to retrieve an authority record from the 'authority' table by email.
   * Parameters: A_mail
   */
  public static final String GET_AUTHORITY_BY_MAIL = "SELECT * FROM authority WHERE mail=?";
}
